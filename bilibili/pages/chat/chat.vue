...
export default {
	...
	data() {
		return {
			...
			keywordReplies: [
				{
					pattern: /你好|嗨|hi|hello|hey/,
					replies: ['你好呀！很高兴和你聊天~', '嗨！有什么可以帮你的吗？', 'hello~ 见到你真开心！']
				},
				{
					pattern: /价格|多少钱|收费|付费|费用/,
					replies: ['目前所有服务都是免费的哦！', '完全免费，放心使用~', '不收费的，尽管问吧！']
				},
				{
					pattern: /下载|链接|安装包|资源/,
					replies: ['链接已经发给你了，请查收~', '我看看... 下载链接在这里！', '资源已分享，快去下载吧！']
				},
				{
					pattern: /谢谢|感谢|thx/,
					replies: ['不客气！有需要随时找我~', '应该的~ 祝你生活愉快！', '不用谢！']
				},
				{
					pattern: /学习|课程|教程|考试|考研/,
					replies: ['学习是最值得的投资！', '加油！坚持就是胜利~', '有问题随时问我，我尽量帮你~']
				},
				{
					pattern: /推荐|建议/,
					replies: ['如果你有兴趣，我可以给你一些建议~', '我推荐你试试这个...', '好的，我帮你留意一下~']
				}
				// 你可以继续添加更多规则
			],
			// 通用回复池（关键词未命中时随机使用）
	...
	methods: {
		...
		sendMessage() {
			...
			if (this.chatInfo.id !== 7 && this.chatInfo.id !== 2) {
				...
				// 1. 先尝试智能匹配（关键词）
				const smartReply = this.getSmartReply(text)
				// 判断是否命中了关键词（即不是从 defaultReplies 中取出的）
				// 如果 smartReply 在 keywordReplies 的回复数组中，说明是关键词匹配；否则是通用回复。
				// 简单判断：如果 smartReply 是通用回复池中的内容，则视为未命中关键词。
				const isKeywordMatch = !this.defaultReplies.includes(smartReply)
				...
				if (isKeywordMatch) {
					...
					// 未命中关键词，检查是否有专属回复（针对特定 ID）
					const specialReply = this.getSpecialReply(this.chatInfo.id)
					if (specialReply) {
						this.isTyping = true
						setTimeout(() => {
							this.isTyping = false
						...
						// 既无关键词匹配，也无专属回复，使用通用回复（即刚才的 smartReply）
						this.isTyping = true
						setTimeout(() => {
							this.isTyping = false
							this.messages.push({
		...
		// 获取专属回复（原 autoReplies 内容）
		getSpecialReply(chatId) {
			const specialMap = {
				3: `LR软件安装包\n下载链接：\nhttps://pan.xunlei.com/s/VO58Ixo2-0gcywyobHgIhhDOA1\n提取码：w53i\n\n剪映软件安装包\n下载链接：\nhttps://pan.xunlei.com/s/VOGZzJuR-zFxC5b3A8YAhdeGA1\n提取码：whep\n\nPR软件安装包\n下载链接：\nhttps://pan.xunlei.com/s/VONcl-jBE6pZAANBN_OYCAaVA1#\n提取码：3cpx\n\nAE软件安装包\n下载链接：\nhttps://pan.xunlei.com/s/VO58IymuD8fePTodZ5hJAleA1\n提取码：6knr\n\nPS软件安装包\n下载链接：\nhttps://pan.quark.cn/s/2c1b24ae0998\n提取码：ffjr`,
				4: '感谢您的关注',
				5: `【粉丝福利：领优惠券】\n1天搞定6000单词！考纲全覆盖！\n方法大于努力，课程差距大！\n错学十年，不如一天。\n跟随光速，最后一次背单词！\n\n选拔性考试：\n中高考、专升本、考研\n能力性考试：\n四六级、专四八、出国\n都需要单词量作为基础。\n单词量=数量+质量\n——————————\n报名在公众号：爱学习的单词君\nwx：gsxdc001`,
				6: `因B站上无法直接发下载链接，所以需要获取安装包链接的，\n请关注微信公众号：\nPSCC12345\n关注后回复：PS下载，就可以获得安装包下载链接了。`,
				8: null, // ID 8 已在 sendMessage 中单独处理
				9: '哈喽，正在备考25考研的同学...\n此条消息为自动回复'
			}
			if (chatId === 8) return null
			return specialMap[chatId] || null
		},

		// 智能回复：根据用户输入匹配关键词，否则返回通用
		getSmartReply(userText) {
			for (let rule of this.keywordReplies) {
				if (rule.pattern.test(userText)) {
					const replies = rule.replies
					return replies[Math.floor(Math.random() * replies.length)]
				}
			}
			const defaultPool = this.defaultReplies
